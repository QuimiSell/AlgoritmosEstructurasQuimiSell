#!/usr/bin/env python3
"""Genera courses/algorithmicComplexityCourse.ts con 20 módulos (15 Big-O + 5 IA)."""
...
MODULES = [
    {
        ...
        "code": """# Comparación empírica vs análisis teórico
import time
...
# Sin importar si la CPU es Intel o Apple Silicon""",
        "quiz": [
            ("¿Qué mide principalmente el análisis de algoritmos con notación Big-O?", ["Segundos exactos en un procesador específico", "Crecimiento del número de operaciones respecto al tamaño de entrada n", "Líneas de código del programa"], 1, "Big-O describe el comportamiento asintótico (cuando n tiende a infinito), no el tiempo absoluto en un hardware concreto."),
            ("Si duplicamos n en un algoritmo O(n), ¿cómo crece el trabajo teórico?", ["Se cuadruplica", "Se duplica aproximadamente", "Permanece igual"], 1, "En O(n) el costo es proporcional a n; duplicar la entrada duplica las operaciones."),
            ("¿Por qué descartamos constantes como 3n → O(n)?", ["Porque Python las optimiza automáticamente", "Porque para n muy grande solo importa el término dominante", "Porque las constantes son ilegales en notación Big-O"], 1, "Asintóticamente, 3n y n crecen con la misma forma; la constante no cambia la clase de complejidad."),
            ("¿Cuál es la unidad típica que contamos en análisis de tiempo?", ["Megabytes de RAM", "Operaciones elementales (comparaciones, asignaciones)", "Número de archivos abiertos"], 1, "Contamos operaciones primitivas del modelo RAM, independientes del lenguaje."),
            ("Un algoritmo O(1) significa que...", ["Nunca usa memoria", "El costo no crece con n (tiempo constante)", "Solo funciona con arreglos"], 1, "O(1) indica que el número de operaciones permanece acotado aunque n aumente."),
        ],
    },
    {
        "id": 2,
        ...
        "code": """# Demostración intuitiva: 3n + 10 ∈ O(n)
def T(n):
    return 3 * n + 10
...
def ejemplo_cuadratico(arr):
    ...
    return count""",
        "quiz": [
            ("¿Qué representa la notación O(f(n))?", ["Igualdad exacta T(n) = f(n)", "Cota superior asintótica del costo", "Cota inferior del costo"], 1, "O(f(n)) significa que T(n) crece como máximo como f(n) multiplicado por una constante."),
            ("¿T(n) = 5n + 200 pertenece a O(n)?", ["Sí", "No, es O(5n)", "No, es O(200)"], 0, "5n + 200 ≤ 6n para n ≥ 200, por lo tanto es O(n)."),
            ("¿Cuál crece más rápido asintóticamente?", ["O(n log n)", "O(n²)", "O(log n)"], 1, "n² supera a n log n y log n para n grande."),
            ("Si T(n) ≤ 2n² + 3n para todo n ≥ 1, ¿cuál es la notación más ajustada como cota superior típica?", ["O(n)", "O(n²)", "O(2n² + 3n)"], 1, "El término dominante es n²; simplificamos a O(n²)."),
            ("¿Por qué usamos O en lugar de tiempo en segundos?", ["Porque los segundos no existen en computadoras", "Porque es independiente del hardware y permite comparar algoritmos", "Porque Python no tiene reloj"], 1, "Big-O abstrae el hardware y permite comparación objetiva entre algoritmos."),
        ],
    },
    {
        "id": 3,
        ...
        "code": """# Búsqueda lineal del máximo: Θ(n)
def maximo(arr):
    if not arr:
        return None
    m = arr[0]
...
# necesita Ω(n) comparaciones en el peor caso""",
        "quiz": [
            ("¿Qué significa T(n) ∈ Ω(f(n))?", ["T(n) crece al menos tan rápido como f(n)", "T(n) es siempre menor que f(n)", "T(n) es exactamente f(n)"], 0, "Ω es cota inferior: T(n) ≥ c·f(n) para n grande."),
            ("Si un algoritmo es O(n) y Ω(n), entonces es...", ["O(n²)", "Θ(n)", "O(1)"], 1, "Cuando coinciden cota superior e inferior, tenemos Θ."),
            ("Merge Sort tiene complejidad...", ["O(n) peor caso", "Θ(n log n) en peor, mejor y promedio", "Ω(n²)"], 1, "Merge Sort siempre divide y combina en n log n."),
            ("¿Qué establece el límite Ω(n log n) para ordenamiento por comparación?", ["Que Quick Sort es imposible", "Que ningún algoritmo basado solo en comparaciones puede hacer mejor que n log n", "Que Bubble Sort es óptimo"], 1, "Es un teorema clásico: comparar elementos requiere al menos n log n comparaciones."),
            ("Un algoritmo con mejor caso O(1) y peor caso O(n) puede tener...", ["Θ(1)", "Ω(1) y O(n) sin ser Θ de ninguno solo", "Siempre Θ(n)"], 1, "Las cotas pueden diferir; Θ requiere mismo orden arriba y abajo en todos los casos analizados."),
        ],
    },
    {
        "id": 4,
        ...
        "code": """# Quick Sort: peor caso O(n²), promedio O(n log n)
def quicksort(arr):
    if len(arr) <= 1:
        return arr
...
# genera particiones desbalanceadas → O(n²)""",
        "quiz": [
            ("¿Qué caso analiza la industria para SLAs de latencia máxima?", ["Best case", "Worst case", "Solo average case"], 1, "Los SLAs garantizan límites superiores; eso es peor caso."),
            ("Insertion Sort en arreglo ya ordenado es...", ["O(n²)", "O(n log n)", "O(n)"], 2, "Cada elemento se compara una vez y no hay desplazamientos grandes."),
            ("Quick Sort sin randomización puede degradarse a O(n²) cuando...", ["El arreglo es aleatorio", "El pivote siempre es el extremo mínimo o máximo", "Usa memoria extra"], 1, "Particiones desbalanceadas en cada nivel producen recursión profunda n."),
            ("El average case requiere...", ["Una sola entrada fija", "Un modelo de distribución de entradas (ej. uniforme)", "Ignorar probabilidades"], 1, "Se calcula E[T(n)] sobre un espacio de entradas con probabilidades."),
            ("Bubble Sort best case con flag de 'swapped' puede ser...", ["O(n) si ya está ordenado", "O(1) siempre", "O(n log n)"], 0, "Una pasada detecta que no hubo intercambios y termina."),
        ],
    },
    {
        "id": 5,
        ...
        "code": """# Ejemplo: simplificar T(n) = 2n² + 50n + log(n) + 1000
# Dominante: n² → O(n²)
...
# log₂(n) y log₁₀(n) son ambos O(log n)""",
        "quiz": [
            ("T(n) = 10n⁴ + n² + 5000 es...", ["O(n²)", "O(n⁴)", "O(5000)"], 1, "El término de mayor grado es n⁴."),
            ("Secuencia: bucle O(n) + bucle O(n log n) = ?", ["O(n)", "O(n log n)", "O(n² log n)"], 1, "Se toma el máximo: n log n domina a n."),
            ("Bucles anidados: for i in n, for j in n, for k in n →", ["O(n)", "O(n³)", "O(3n)"], 1, "Producto de tres factores n: n³."),
            ("¿2ⁿ + n¹⁰⁰ pertenece a O(2ⁿ)?", ["Sí", "No, es O(n¹⁰⁰)", "No, es O(1)"], 0, "Exponencial domina cualquier polinomio."),
            ("log₂(n) y log₁₀(n) en Big-O son...", ["O(log n) ambos", "O(log n) y O(1) respectivamente", "Incomparables"], 0, "Cambiar base del logaritmo solo multiplica por constante."),
        ],
    },
    {
        "id": 6,
...
def potencia_dos(n):
    ...
    return ops""",
        "quiz": [
            ("for i in range(n): for j in range(n): →", ["O(n)", "O(n²)", "O(log n)"], 1, "n × n iteraciones."),
            ("for i in range(n): for j in range(i): →", ["O(n)", "O(n²)", "O(n log n)"], 1, "Suma triangular n(n-1)/2 ∈ O(n²)."),
            ("while n > 1: n = n // 2 →", ["O(n)", "O(log n)", "O(1)"], 1, "n se reduce a la mitad cada iteración."),
            ("Dos bucles consecutivos, cada uno O(n), total...", ["O(n)", "O(n²)", "O(2n) simplificado a O(n)"], 2, "Secuencia: máximo O(n); 2n sigue siendo O(n)."),
            ("Técnica de dos punteros en arreglo ordenado típicamente es...", ["O(n²)", "O(n)", "O(n log n)"], 1, "Cada puntero avanza como máximo n veces en total."),
        ],
    },
    {
        "id": 7,
        ...
        "code": """# Fibonacci ingenuo: O(2ⁿ) tiempo, O(n) espacio de pila
def fib(n):
    if n <= 1:
        return n
...
def merge_sort(arr):
    ...
    return merge(L, R)  # merge lineal O(n)""",
        "quiz": [
            ("Fibonacci recursivo sin memoización tiene complejidad temporal...", ["O(n)", "O(2ⁿ)", "O(log n)"], 1, "El árbol de llamadas se ramifica exponencialmente."),
            ("Merge Sort satisface T(n) = 2T(n/2) + O(n). Por Teorema Maestro es...", ["O(n)", "O(n log n)", "O(n²)"], 1, "Caso clásico del teorema: f(n)=n, log_b a = 1."),
            ("La profundidad de recursión de binary search en n elementos es...", ["O(n)", "O(log n)", "O(1)"], 1, "Se divide el espacio a la mitad cada llamada."),
            ("¿Qué problema tiene fib(1000) recursivo ingenuo?", ["Usa poca memoria", "Stack overflow y tiempo prohibitivo", "Es O(1)"], 1, "Profundidad n y tiempo exponencial."),
            ("Memoización convierte fib de O(2ⁿ) a...", ["O(n) tiempo y O(n) espacio", "O(1)", "O(n²)"], 0, "Cada subproblema se calcula una sola vez."),
        ],
    },
    {
        "id": 8,
...
cache["user:42"]""",
        "quiz": [
            ("Acceder a arr[k] en un arreglo de tamaño n es...", ["O(n)", "O(1)", "O(log n)"], 1, "Direccionamiento directo por índice."),
            ("push/pop en stack (lista Python) es...", ["O(n) siempre", "O(1) amortizado", "O(log n)"], 1, "append/pop al final son constantes amortizadas."),
            ("O(1) significa que el algoritmo...", ["No usa memoria", "Hace trabajo acotado independiente de n", "Siempre tarda nanosegundos"], 1, "Acotado, no necesariamente microscópico."),
            ("¿Cuál operación NO es O(1) en lista Python?", ["arr[0]", "arr.append(x)", "arr.insert(0, x)"], 2, "Insertar al inicio desplaza n elementos → O(n)."),
            ("Tabla hash con buena función y load factor bajo: búsqueda promedio...", ["O(1)", "O(n)", "O(log n)"], 0, "Dispersión uniforme minimiza colisiones."),
        ],
    },
    {
        "id": 9,
        ...
        "code": """def busqueda_binaria(arr, objetivo):
    izq, der = 0, len(arr) - 1
    pasos = 0
    while izq <= der:
        pasos += 1
...
# n=1_000_000 → pasos ≈ log2(1e6) ≈ 20""",
        "quiz": [
            ("Búsqueda binaria requiere que el arreglo esté...", ["Ordenado", "Sin duplicados obligatoriamente", "En lista enlazada"], 0, "La monotonicidad permite descartar mitades."),
            ("Para n = 1,048,576 (2²⁰), búsqueda binaria hace como máximo ~...", ["20 comparaciones", "1 millón de comparaciones", "2 comparaciones"], 0, "log₂(2²⁰) = 20."),
            ("Inserción en AVL tree balanceado es...", ["O(n)", "O(log n)", "O(1)"], 1, "Altura del árbol es O(log n)."),
            ("Duplicar n en algoritmo O(log n) agrega...", ["El doble de pasos", "Un paso constante extra", "n pasos extra"], 1, "log(2n) = log(n) + 1."),
            ("¿Cuál NO es típicamente O(log n)?", ["Búsqueda binaria", "Recorrido completo de arreglo", "Búsqueda en BST balanceado"], 1, "Recorrer todo el arreglo es O(n)."),
        ],
    },
    {
        "id": 10,
...
#     for j in range(i+1, n): ...""",
        "quiz": [
            ("Encontrar el máximo de un arreglo desordenado es...", ["O(1)", "O(n)", "O(log n)"], 1, "Debes inspeccionar cada elemento al menos una vez (Ω(n))."),
            ("Copiar un arreglo de n elementos cuesta...", ["O(1)", "O(n)", "O(n²)"], 1, "n asignaciones individuales."),
            ("Two-sum con diccionario es O(n) porque...", ["Usa recursión", "Una pasada con lookup O(1) promedio", "Ordena primero"], 1, "Un bucle + hash evita el par anidado."),
            ("Tres bucles secuenciales O(n) cada uno = ...", ["O(n)", "O(n³)", "O(3n) = O(n)"], 0, "Secuencia: sigue siendo O(n)."),
            ("Procesar un archivo de n líneas línea a línea es...", ["O(n)", "O(n²)", "O(log n)"], 0, "Una operación por línea."),
        ],
    },
    {
        "id": 11,
...
# log n niveles × O(n) merge por nivel""",
        "quiz": [
            ("¿Cuál ordenamiento garantiza O(n log n) en peor caso?", ["Quick Sort básico", "Merge Sort", "Bubble Sort"], 1, "Merge Sort no depende del orden de entrada."),
            ("¿Por qué Merge Sort es O(n log n)?", ["Un solo bucle n", "log n niveles de división × O(n) merge", "n bucles anidados"], 1, "Estructura clásica divide y vencerás."),
            ("El límite Ω(n log n) para sorting por comparación implica...", ["Merge Sort es óptimo en ese modelo", "O(n) sorting es posible siempre", "Solo aplica a enteros"], 0, "No se puede comparar menos en el peor caso."),
            ("Quick Sort en producción suele ser O(n log n) promedio porque...", ["Siempre elige pivote mediano exacto", "Pivote aleatorio evita entradas adversas típicas", "No usa recursión"], 1, "Randomización reduce probabilidad de peor caso."),
            ("Heap Sort tiene complejidad...", ["O(n)", "O(n log n)", "O(n²)"], 1, "n extracciones × O(log n) cada una."),
        ],
    },
    {
        "id": 12,
    ...
    for i in range(n):
        for j in range(n):
            for k in range(n):
                C[i][j] += A[i][k] * B[k][j]""",
        "quiz": [
            ("Bubble Sort peor caso es...", ["O(n)", "O(n log n)", "O(n²)"], 2, "Dos bucles anidados sobre n."),
            ("Multiplicación matricial n×n ingenua es...", ["O(n²)", "O(n³)", "O(n log n)"], 1, "Tres bucles i, j, k."),
            ("Con n=20,000, O(n²) implica del orden de...", ["400 millones de operaciones", "20,000 operaciones", "20 operaciones"], 0, "(2×10⁴)² = 4×10⁸."),
            ("¿Cuándo O(n²) puede ser aceptable?", ["n siempre > 1 millón", "n acotado pequeño (ej. ≤ 500)", "Nunca"], 1, "Constantes pequeñas y n limitado."),
            ("Floyd-Warshall para caminos mínimos en grafo denso es...", ["O(V²)", "O(V³)", "O(V log V)"], 1, "Triple bucle sobre vértices."),
        ],
    },
    {
        "id": 13,
        ...
        "code": """# Generar todos los subconjuntos: O(2ⁿ)
...
# n=40 → 2^40 ≈ 1 billón (imposible)""",
        "quiz": [
            ("Generar todos los subconjuntos de n elementos es...", ["O(n)", "O(2ⁿ)", "O(n²)"], 1, "2ⁿ combinaciones posibles."),
            ("Permutaciones de n elementos son...", ["O(n!)", "O(2ⁿ)", "O(n log n)"], 0, "n! ordenamientos distintos."),
            ("Fibonacci recursivo sin memo es O(2ⁿ) porque...", ["Cada llamada hace dos subllamadas", "Usa un bucle", "Es divide and conquer log"], 0, "Árbol binario de llamadas."),
            ("TSP fuerza bruta con n ciudades es...", ["O(n²)", "O(n!)", "O(n log n)"], 1, "Debe evaluar permutaciones de rutas."),
            ("Para n=35, 2ⁿ es aproximadamente...", ["35 operaciones", "34 mil millones", "Imposible de calcular en tiempo humano razonable"], 1, "2³⁵ ≈ 34×10⁹."),
        ],
    },
    {
        "id": 14,
    ...
    def _resize(self, new_cap):
        ...
        "quiz": [
            ("Append en dynamic array con duplicación es...", ["O(n) siempre", "O(1) amortizado", "O(log n)"], 1, "Copias raras; n appends cuestan O(n) total."),
            ("Análisis amortizado estudia...", ["Solo una operación aislada", "Costo promedio por operación en secuencia larga", "Solo best case"], 1, "Promedio sobre secuencia, no expectativa aleatoria."),
            ("¿Una operación amortizada O(1) puede costar O(n) una vez?", ["Sí, ocasionalmente", "No, nunca", "Solo en best case"], 0, "El peor caso puntual puede ser alto; el promedio bajo."),
            ("Union-Find con path compression tiene find casi...", ["O(1) amortizado", "O(n!) siempre", "O(n²)"], 0, "Estructura clásica de amortización."),
            ("Si n appends cuestan O(n) total, cada append es...", ["O(1) amortizado", "O(n) amortizado", "O(n²)"], 0, "O(n)/n = O(1) por operación."),
        ],
    },
    {
        "id": 15,
        ...
        "code": """# Fibonacci: trade-off tiempo vs espacio
...
def fib_iter(n):
    ...
    return a""",
        "quiz": [
            ("Merge Sort usa espacio auxiliar...", ["O(1)", "O(n)", "O(n²)"], 1, "Array temporal para merge."),
            ("Memoización típicamente intercambia...", ["Tiempo por más espacio", "Espacio por más tiempo", "Nada"], 0, "Guarda resultados para evitar recomputar."),
            ("fib iterativo vs memo: el iterativo usa espacio...", ["O(1)", "O(2ⁿ)", "O(n log n)"], 0, "Solo dos variables."),
            ("¿Big-O reemplaza profiling en producción?", ["Sí, siempre", "No, se complementan", "Solo para Python"], 1, "Constantes y caché importan en hardware real."),
            ("Quick Sort in-place tiene stack recursivo...", ["O(n) peor caso", "O(1) siempre", "O(n²) espacio"], 0, "Recursión profunda en peor caso."),
        ],
    },
    # --- 5 módulos IA ---
    {
...
print(capa_densa_flops(32, 768, 3072))  # ~151 millones FLOPs""",
        "quiz": [
            ("Capa densa batch×in×out tiene complejidad de multiplicaciones...", ["O(batch · in · out)", "O(in + out)", "O(1)"], 0, "Producto de dimensiones de la matriz."),
            ("Duplicar batch size en inferencia duplica...", ["Solo memoria, no FLOPs", "FLOPs linealmente", "FLOPs cuadráticamente"], 1, "Costo proporcional al batch."),
            ("ReLU sobre tensor de n elementos es...", ["O(n)", "O(n²)", "O(log n)"], 0, "Una operación por elemento."),
            ("¿Más parámetros siempre implica más FLOPs por token?", ["Sí siempre", "Depende de arquitectura y capas activas", "No, nunca"], 1, "MoE activa subconjuntos; depth vs width importa."),
            ("FLOPs miden...", ["Operaciones de punto flotante", "Bytes transferidos", "Solo memoria GPU"], 0, "Métrica estándar de costo computacional en IA."),
        ],
    },
    {
        "id": 17,
        ...
        "code": """# ❌ Gauss-Jordan: difícil de paralelizar (dependencias fila a fila)
...
Y = X @ W   # O(batch·in·out) pero paralelismo masivo""",
        "quiz": [
            ("¿Por qué Gauss-Jordan es malo en GPUs para entrenamiento?", ["Es O(n²)", "Tiene dependencias secuenciales entre filas difíciles de paralelizar", "No existe en matemáticas"], 1, "SIMT requiere operaciones independientes masivas."),
            ("El hot path del entrenamiento de redes usa principalmente...", ["Multiplicación matricial GEMM", "Eliminación gaussiana", "Ordenamiento burbuja"], 0, "Forward/backward son cadenas de matmul."),
            ("GEMM y Gauss-Jordan para n×n son ambos O(n³), pero GEMM gana porque...", ["Tiene mejor paralelismo y kernels optimizados", "Usa menos memoria siempre", "Es O(n²)"], 0, "Constantes y paralelismo, no solo exponente."),
            ("Tensor Cores en NVIDIA aceleran especialmente...", ["Operaciones matriciales mix-precision", "Parsing JSON", "I/O de disco"], 0, "Diseñados para GEMM en FP16/BF16."),
            ("PyTorch invierte matrices ocasionalmente con métodos numéricos estables, pero el entrenamiento masivo usa...", ["CuBLAS GEMM", "Gauss-Jordan manual en Python puro", "Búsqueda lineal"], 0, "Librerías BLAS/CUDA optimizadas."),
        ],
    },
    {
        "id": 18,
        ...
        "code": """# Regla práctica: FLOPs entrenamiento ≈ 3 × FLOPs inferencia
...
# Gradient checkpointing trade-off: menos memoria, más recomputación""",
        "quiz": [
            ("Backpropagation agrega aproximadamente...", ["0× FLOPs", "2× FLOPs del forward", "100× FLOPs"], 1, "Derivadas vía regla de la cadena."),
            ("Entrenamiento por step ≈ ... × inferencia forward", ["1×", "3×", "100×"], 1, "Forward + backward + optimizer."),
            ("Activaciones intermedias se guardan para...", ["Calcular gradientes en backward", "Solo visualización", "Comprimir el modelo"], 0, "Reverse mode necesita valores del forward."),
            ("Inferencia en producción típicamente ejecuta...", ["Solo forward pass", "Forward + backward + optimizer", "Solo backward"], 0, "No hay gradientes en despliegue."),
            ("Gradient checkpointing intercambia...", ["Memoria por recomputación (más FLOPs)", "Precisión por velocidad siempre", "GPU por CPU"], 0, "Recalcula activaciones para ahorrar RAM."),
        ],
    },
    {
        "id": 19,
        ...
        "code": """# Self-Attention simplificado
...
# n=4096 → n²=16M por cabeza; multiplicar por L capas""",
        "quiz": [
            ("Self-attention clásica en secuencia n tiene complejidad...", ["O(n)", "O(n log n)", "O(n²·d)"], 2, "Matriz de scores n×n por dimensión."),
            ("¿Por qué contexto 128K es caro en Transformers vanilla?", ["O(n) lineal", "Memoria y compute O(n²)", "O(1)"], 1, "n² crece explosivamente."),
            ("FlashAttention mejora principalmente...", ["Constantes IO y memoria HBM, no el exponente de n", "Cambia O(n²) a O(n)", "Elimina softmax"], 0, "IO-aware; sigue siendo cuadrático."),
            ("Sliding window attention reduce complejidad a...", ["O(n·w) con ventana w", "O(n²)", "O(1)"], 0, "Cada token atiende solo w vecinos."),
            ("QK^T produce matriz de forma...", ["(n, n)", "(d, d)", "(n, d) solo"], 0, "Scores entre todos los pares de tokens."),
        ],
    },
    {
        "id": 20,
        ...
        "code": """# Autoregresión CON KV-cache (estándar en vLLM, llama.cpp)
...
# 70B params: 280 GB → ~35 GB (ideal teórico)""",
        "quiz": [
            ("KV-cache en inferencia LLM evita...", ["Recomputar keys/values de tokens anteriores", "Usar GPU", "El forward pass"], 0, "Solo calcula Q del token nuevo contra K,V cacheados."),
            ("Cuantización INT4 reduce principalmente...", ["Uso de memoria y bandwidth", "Big-O de atención de n² a n", "Número de capas"], 0, "Menos bytes por peso; mismo algoritmo."),
            ("Batching en inferencia aumenta throughput porque...", ["Amortiza overhead kernel sobre muchas secuencias", "Cambia O(n²) a O(n)", "Elimina softmax"], 0, "GPUs prefieren operaciones grandes."),
            ("Tensor parallelism divide...", ["Matrices grandes entre GPUs", "Solo el dataset", "La loss function"], 0, "Shard de capas d×d en múltiples dispositivos."),
            ("Speculative decoding usa modelo pequeño para...", ["Proponer tokens verificados por modelo grande", "Entrenar desde cero", "Eliminar atención"], 0, "Reduce forward passes del modelo grande."),
        ],
    },
]

STUDY_PLAN = [
    (1, "Fundamentos del Análisis", "Establece el lenguaje matemático para medir eficiencia antes de codificar."),
    (2, "Notación O, Ω y Θ", "Domina cotas superiores, inferiores y ajustadas para garantías formales."),
    (3, "Casos y Simplificación", "Best/worst/average case y reglas algebraicas de simplificación."),
    (4, "Bucles y Recursión", "Patrones iterativos, recurrencias y Teorema Maestro."),
    (5, "Clases Clásicas O(1) a O(n!)", "Catálogo completo de complejidades con ejemplos reales."),
    (6, "Amortizado y Trade-offs", "Dynamic arrays, tiempo vs espacio y selección de algoritmos."),
    (7, "Big-O en Redes Neuronales", "FLOPs en capas densas, convoluciones y forward pass."),
    (8, "GPUs y Álgebra Lineal", "Por qué GEMM gana sobre Gauss-Jordan en entrenamiento masivo."),
    (9, "Transformers y Optimización", "Atención cuadrática, KV-cache, cuantización y paralelismo."),
]
...
def main():
    ...
    out = f"""import {{ Course }} from '../domain/models';
...
"""
    path = Path(__file__).resolve().parent.parent / "courses" / "algorithmicComplexityCourse.ts"
